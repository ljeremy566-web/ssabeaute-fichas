...
export const EmptyState = ({ icon, title, description, actionLabel, onAction, className }: EmptyStateProps) => (
  <div className={cn('py-16 px-6 text-center', className)}>
    ...
    <h3 className="text-lg font-bold text-on-surface font-outfit mb-2">{title}</h3>
    {description && (
      <p className="text-sm text-on-surface-variant font-medium max-w-sm mx-auto mb-6">{description}</p>
    )}
